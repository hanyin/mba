...
import com.hy.mvp.ui.adapter.ChatAdapter;
import com.hy.mvp.ui.adapter.FaceVPAdapter;
import com.hy.mvp.ui.bean.Msg;
import com.hy.mvp.ui.bean.Session;
import com.hy.mvp.ui.view.DropdownListView;
import com.hy.mvp.utils.CommonUtil;
import com.hy.mvp.utils.ExpressionUtil;
import com.hy.mvp.utils.PreferencesUtils;
import com.hy.mvp.utils.XmppUtil;
...
public class ChatActivity extends Activity implements OnClickListener,DropdownListView.OnRefreshListenerHeader {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		I= AppConfigHelper.getConfig(AppConfigDef.userId);
	...
	public void initData(){
		offset=0;
		listMsg=msgDao.queryMsg(I,YOU,offset);
		offset=listMsg.size();
	...
	void sendMsgText(String content){
		...
		mLvAdapter.notifyDataSetChanged();
		input.setText("");
		final String message=I+Const.SPLIT+YOU+Const.SPLIT+Const.MSG_TYPE_TEXT+Const.SPLIT+content+Const.SPLIT+sd.format(new Date());
		new Thread(new Runnable() {
			@Override
			public void run() {
				try {
	...
	public void onRefresh() {
		List<Msg> list=msgDao.queryMsg(I,YOU,offset);
		if(list.size()<=0){
	...
	private class MsgOperReciver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			Msg msger = intent.getParcelableExtra("msg");
